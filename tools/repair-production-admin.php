...
$path = __DIR__ . '/../src/Presentation/Admin/ProductionAdmin.php';
$text = file_get_contents($path);
if ($text === false) {
    fwrite(STDERR, "ProductionAdmin.php not found\n");
    exit(1);
}
$start = strpos($text, ' private function scripts(');
$end = $start === false ? false : strpos($text, ' public function save(', $start);
if ($start === false || $end === false) {
    fwrite(STDERR, "ProductionAdmin.php format not recognized\n");
    exit(1);
}
$old = substr($text, $start, $end - $start);
if (strpos($old, "<<<'JS'") !== false) {
    exit(0);
}
...
file_put_contents($path, substr($text, 0, $start) . $replacement . substr($text, $end));

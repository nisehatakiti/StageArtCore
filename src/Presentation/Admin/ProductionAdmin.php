...
(function(){
 ...
 f.addEventListener('submit',function(){
  ...
  document.querySelectorAll('#sa-credits .sa-credit').forEach(function(box){
   const nameInput=box.querySelector('input[data-credit-field="name"]');
   const releaseInput=box.querySelector('input[data-credit-field="release_enabled"]');
   ...
   if(!nameInput)return;
   const items=[];
   box.querySelectorAll('.sa-credit-item input[data-credit-item-field="name"]').forEach(function(input){
    const name=(input.value||'').trim();
    if(name!=='')items.push(name);
   });
   payload.push({
    id:sectionId?parseInt(sectionId.value||'0',10):0,
    name:nameInput.value||'',
    release_enabled:!!(releaseInput&&releaseInput.checked),
    release_at:releaseAt?(releaseAt.value||''):'',
    items:items
   });
  });
  const jsonInput=document.getElementById('sa-credits-json');

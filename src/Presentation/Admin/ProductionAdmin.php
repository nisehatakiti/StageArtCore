...
.sa-credit-items{margin:10px 0}.sa-credit-item-list{display:flex;flex-direction:column;gap:8px}.sa-credit-item{display:flex;align-items:center;gap:8px}.sa-credit-item input:not([type=hidden]){width:min(520px,100%)}.sa-credit-item input[data-credit-item-field="url"]{width:min(520px,100%)}.sa-credit-item .sa-remove-item{white-space:nowrap}.sa-credit-item-add{margin-top:8px}
 ...
 {
  echo '<style>
  #stageart-production-tabs{margin:20px 0 0}
  #stageart-production-tabs .nav-tab{cursor:pointer}
  .sa-production-pane{background:#fff;padding:20px;border:1px solid #c3c4c7;border-top:0;margin-bottom:20px}
  #stageart-production-pane-display{display:none}
  ...
  document.addEventListener("DOMContentLoaded",function(){
    ...
    const tabs=form.querySelectorAll("#stageart-production-tabs [data-sa-tab]");
    const panes={
      basic:document.getElementById("stageart-production-pane-basic"),
      content:document.getElementById("stageart-production-pane-content"),
      display:document.getElementById("stageart-production-pane-display")
    };
    function show(key){
      Object.keys(panes).forEach(function(k){
        if(panes[k])panes[k].style.display=k===key?"block":"none";
      });
      tabs.forEach(function(tab){tab.classList.toggle("nav-tab-active",tab.dataset.saTab===key);});
      try{localStorage.setItem("stageart-production-tab",key);}catch(e){}
    }
    tabs.forEach(function(tab){tab.addEventListener("click",function(){show(tab.dataset.saTab);});});
    let initial="basic";
    try{initial=localStorage.getItem("stageart-production-tab")||"basic";}catch(e){}
    if(!panes[initial])initial="basic";
    show(initial);
  });
  </script>';
  $labelOptions='';

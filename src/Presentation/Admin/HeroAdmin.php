...
final class HeroAdmin
{
    ...
    public function script(): void
    {
        ...
        echo '<script>(function(){const form=document.querySelector("form[action*=\\"admin-post.php\\"] input[name=\\"action\\"][value=\\"stageart_save_homepage\\"]")?.form;if(!form)return;const ps='.$presets.',s='.$state.';const b=document.createElement("fieldset");b.className="sa-hero-block";b.innerHTML="<legend><strong>Hero</strong></legend><p class=\\"description\\">団体トップページの独立したHeroブロックです。</p><p><label><input type=\\"checkbox\\" class=\\"sa-hero-enabled\\"> Heroを表示する</label></p><h3>背景画像</h3><p class=\\"description\\">StageArt標準の背景から選択できます。自分で用意した画像はWordPressメディアから選択します。</p><div class=\\"sa-hero-groups\\"></div><p><button type=\\"button\\" class=\\"button sa-hero-media\\">自分で用意した画像を選択</button> <button type=\\"button\\" class=\\"button-link-delete sa-hero-clear\\">自前画像を解除</button></p><div class=\\"sa-hero-preview\\"></div>";const hidden=(n,v)=>{let e=form.querySelector(`[name=\\"${n}\\"]`);if(!e){e=document.createElement("input");e.type="hidden";e.name=n;form.appendChild(e)}e.value=v||"";return e};const en=b.querySelector(".sa-hero-enabled"),groups=b.querySelector(".sa-hero-groups"),preview=b.querySelector(".sa-hero-preview"),ht=hidden("stageart_hero_enabled",s.enabled?"1":"0"),ty=hidden("stageart_hero_background_type",s.type),pr=hidden("stageart_hero_background_preset",s.preset),ur=hidden("stageart_hero_background_url",s.url);en.checked=s.enabled;function previewImg(){preview.innerHTML="";const p=ps.find(x=>x.id===s.preset),src=s.type==="custom"?s.url:(p?.url||"");if(src)preview.innerHTML=`<img src="${src}" alt="Hero背景プレビュー">`}function pick(id){s.type="preset";s.preset=id;s.url="";ty.value="preset";pr.value=id;ur.value="";groups.querySelectorAll("button.sa-hero-preset").forEach(x=>x.classList.toggle("is-selected",x.dataset.id===id));previewImg()}const grouped={};ps.forEach(x=>(grouped[x.category]??=[]).push(x));Object.entries(grouped).forEach(([category,items])=>{const g=document.createElement("div");g.className="sa-hero-group";g.innerHTML=`<h4 class="sa-hero-group-title">${category}</h4><div class="sa-hero-presets"></div>`;const grid=g.querySelector(".sa-hero-presets");items.forEach(x=>{const q=document.createElement("button");q.type="button";q.className="sa-hero-preset";q.dataset.id=x.id;q.innerHTML=`<span class="sa-hero-check">✓</span><img src="${x.url}" alt=""><span class="sa-hero-preset-label">${x.label}</span>`;q.onclick=()=>pick(x.id);grid.appendChild(q)});groups.appendChild(g)});if(s.type==="preset")pick(s.preset);else previewImg();en.onchange=()=>{s.enabled=en.checked;ht.value=s.enabled?"1":"0"};b.querySelector(".sa-hero-media").onclick=()=>{if(!window.wp?.media)return;const f=wp.media({title:"Hero背景画像を選択",button:{text:"この画像を使用"},multiple:false,library:{type:"image"}});f.on("select",()=>{const a=f.state().get("selection").first().toJSON();if(!a?.url)return;s.type="custom";s.url=a.url;ty.value="custom";ur.value=a.url;groups.querySelectorAll("button.sa-hero-preset").forEach(x=>x.classList.remove("is-selected"));previewImg()});f.open()};b.querySelector(".sa-hero-clear").onclick=()=>pick(s.preset||ps[0]?.id||"");form.insertBefore(b,form.querySelector("#stageart-home-sections")||form.firstElementChild);form.addEventListener("submit",()=>{ht.value=en.checked?"1":"0";ty.value=s.type;pr.value=s.preset;ur.value=s.url})})();</script>';
    }

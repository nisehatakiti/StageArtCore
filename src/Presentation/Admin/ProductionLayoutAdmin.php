...
final class ProductionLayoutAdmin
{
    ...
    {
        // ProductionAdmin uses the same admin_post action and redirects/exits.
        // Save the layout first so it is persisted before the main production save runs.
    ...
    public function footer(): void
    {
        ...
        echo '<script>(function(){
            ...
            if(!f)return;
            let sections=' . $json . ',labels=' . $labelsJson . ',picker=' . $pickerJson . ';
            ...
            function wire(sec){
                ...
                const update=function(){
                    const count=+cols.value||1;
                    sec.querySelectorAll(".sa-pl-slot").forEach((r,i)=>r.hidden=i>=count)
                };
                cols.addEventListener("change",update);
                update();
                sec.querySelectorAll(".sa-pl-slot").forEach(function(row){
                    const t=row.querySelector("[data-type]"),c=row.querySelector(".sa-pl-content"),h=row.querySelector(".sa-pl-heading");
                    const toggle=function(){
                        const heading=t.value==="heading";
                        c.style.display=heading?"none":"inline-block";
                        h.style.display=heading?"inline-block":"none"
                    };
            ...
            function draw(){
                ...
                sections.forEach(function(s,si){
                    ...
                    for(let i=0;i<20;i++){
                        ...
                        const c=r.querySelector(".sa-pl-content");
                        const h=r.querySelector(".sa-pl-heading");
                        if(heading){h.value=String(sl.ref||"")}else{c.value=sl.ref||""}
                        r.querySelector(".sa-pl-indent").value=String(sl.indent||0);
                        r.querySelector("[data-type]").onchange=function(){sl.type=this.value;if(this.value!=="heading"){sl.ref=c.value||""}sync()};
                        c.onchange=function(){sl.type="link";sl.ref=this.value;sync()};
                        h.oninput=function(){sl.type="heading";sl.ref=this.value;sync()};
                        r.querySelector(".sa-pl-indent").onchange=function(){sl.indent=+this.value;sync()};
                        slots.appendChild(r)
                    }
            ...
            add.querySelector("button").onclick=function(){
                sections.push({section_id:"new-"+Date.now(),heading:"",columns:1,layout:"vertical",slots:[{type:"none",ref:"",indent:0}]});
                draw();sync()
            };
            draw();
            sync();
            const submit=f.querySelector("button[type=submit]");
            if(submit)f.insertBefore(box,submit.parentNode)
        })();</script>';
    }

...
const addBlock=document.createElement("p");addBlock.innerHTML="<button type=\"button\" class=\"button\" id=\"sa-add-block\">＋ コンテンツブロックを追加</button>";box.appendChild(addBlock);\naddBlock.querySelector("#sa-add-block").onclick=()=>{blocks.push({block_id:"new-"+Date.now(),sections:[{section_id:"new-"+Date.now()+"-s",heading:"",columns:3,layout:"horizontal",slots:[{type:"none",ref:"",indent:0}]}]});draw();sync()};\ndraw();sync();

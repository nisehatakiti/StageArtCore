...
final class RepresentativeGreetingAdmin
{
    ...
    public function render(): void
    {
        ...
        echo '<p>対象メンバーの写真・名前・メンバーページへのリンクはメンバー情報から自動取得します。ここでは挨拶本文と情報解禁を管理します。</p><form method="post" action="' . esc_url(admin_url('admin-post.php')) . '">';
        wp_nonce_field('stageart_representative_greeting');
        echo '<input type="hidden" name="action" value="stageart_save_representative_greeting"><table class="form-table"><tr><th>対象メンバー</th><td><select name="member_id"><option value="0">選択してください</option>';

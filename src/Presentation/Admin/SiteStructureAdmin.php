...
final class SiteStructureAdmin
{
    ...
    public function renderHomepage():void
    {
        ...
        wp_nonce_field('stageart_homepage');echo'<input type="hidden" name="action" value="stageart_save_homepage"><input type="hidden" name="stageart_return_page" value="'.esc_attr($organizationPage?'stageart-organization':'stageart-homepage').'"><input type="hidden" name="stageart_return_tab" value="'.esc_attr($organizationPage?'display':'').'"><div id="stageart-content-blocks">';
...
renumber()})();</script>';
    ...
    public function saveHomepage():void
    {
        ...
        update_option(SiteStructure::HOME_OPTION,$out,false);$returnPage=sanitize_key((string)($_POST['stageart_return_page']??'stageart-homepage'));$returnTab=sanitize_key((string)($_POST['stageart_return_tab']??''));$args=['page'=>$returnPage,'saved'=>'1'];if($returnTab!=='')$args['tab']=$returnTab;wp_safe_redirect(add_query_arg($args,admin_url('admin.php')));exit;
    }

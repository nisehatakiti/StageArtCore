...
final class ProductionRouter
{
    ...
    public function render(): void
    {
        ...
        echo '<section class="stageart-production-hero"><div class="stageart-production-hero-copy"><p class="stageart-production-hero-kicker">StageArt / Production</p><h1 class="stageart-production-hero-title">'.esc_html($p->post_title).'</h1>';
        if($this->released($g('summary_release'))&&$g('summary'))echo '<p class="stageart-production-hero-summary">'.esc_html($g('summary')).'</p>';elseif(!$this->released($g('summary_release')))echo '<p class="stageart-production-hero-summary">近日公開</p>';
        ...
        $layout=ProductionLayout::get($p->ID);foreach($layout as$section){echo '<section class="stageart-production-layout-section">';if(!empty($section['heading']))echo '<h2>'.esc_html($section['heading']).'</h2>';echo '<div class="stageart-production-layout-slots stageart-production-layout-slots--'.esc_attr($section['layout']).'" style="--stageart-layout-columns:'.(int)$section['columns'].'">';foreach((array)$section['slots']as$slot){$type=(string)($slot['type']??'link');$ref=(string)($slot['ref']??'');if($type==='heading'){echo '<div class="stageart-production-layout-slot stageart-production-layout-slot--heading"><h3>'.esc_html($ref).'</h3></div>';continue;}if(!$ref)continue;$size='l';if($ref==='performances')$size=$performanceSize;if($ref==='tickets')$size=$ticketSize;echo '<div class="stageart-production-layout-slot stageart-production-layout-slot--'.esc_attr($ref).' stageart-size-'.esc_attr($size).'">';$this->renderBlock($ref,$p,$g,$r,$c,true);echo '</div>'; }echo '</div></section>';}

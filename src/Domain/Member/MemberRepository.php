...
namespace StageArtCore\Domain\Member;
...
final class MemberRepository
{
    ...
    {
        global $wpdb;
        $t=$wpdb->prefix.'stageart_plugin_members';
        $rows=$wpdb->get_results("SELECT * FROM {$t} ORDER BY display_order ASC,id ASC",ARRAY_A)?:[];
        if(!$publicOnly)return$rows;
        return array_values(array_filter($rows,static fn(array $row):bool => ($row['status']??'')==='published' && ReleaseDate::isReleased($row['release_at']??null)));
    }
    ...
    public function findBySlugHistory(string $slug):?array{global $wpdb;$h=$wpdb->prefix.'stageart_plugin_member_slug_history';$id=(int)$wpdb->get_var($wpdb->prepare("SELECT member_id FROM {$h} WHERE slug=%s",$slug));return$id?$this->find($id):null;}
    public function currentSlug(int $id):?string{global $wpdb;$t=$wpdb->prefix.'stageart_plugin_members';$slug=$wpdb->get_var($wpdb->prepare("SELECT slug FROM {$t} WHERE id=%d",$id));return$slug!==null?(string)$slug:null;}
    public function save(array$d):int{global $wpdb;$t=$wpdb->prefix.'stageart_plugin_members';$now=current_time('mysql');$id=(int)($d['id']??0);$name=sanitize_text_field((string)($d['name']??''));$slug=$this->uniqueSlug(sanitize_title((string)($d['slug']??$name)),$id);$photo=!empty($d['photo_id'])?(int)$d['photo_id']:null;$profile=wp_kses_post((string)($d['profile']??''));$status=in_array(($d['status']??'draft'),['draft','published'],true)?$d['status']:'draft';$order=max(0,(int)($d['display_order']??0));$release=$d['release_at']??null;$release=$release!==null&&trim((string)$release)!==''?(string)$release:null;if($id){$old=(string)$wpdb->get_var($wpdb->prepare("SELECT slug FROM {$t} WHERE id=%d",$id));$wpdb->update($t,['name'=>$name,'slug'=>$slug,'photo_id'=>$photo,'profile'=>$profile,'status'=>$status,'display_order'=>$order,'release_at'=>$release,'updated_at'=>$now],['id'=>$id],['%s','%s','%d','%s','%s','%d','%s','%s'],['%d']);if($old&&$old!==$slug)$wpdb->insert($wpdb->prefix.'stageart_plugin_member_slug_history',['member_id'=>$id,'slug'=>$old,'created_at'=>$now],['%d','%s','%s']);}else{$wpdb->insert($t,['name'=>$name,'slug'=>$slug,'photo_id'=>$photo,'profile'=>$profile,'status'=>$status,'display_order'=>$order,'release_at'=>$release,'created_at'=>$now,'updated_at'=>$now],['%s','%s','%d','%s','%s','%d','%s','%s','%s']);$id=(int)$wpdb->insert_id;}$this->replaceRoles($id,(array)($d['roles']??[]));$this->replaceSocial($id,(array)($d['social']??[]));$this->replaceCustomValues($id,(array)($d['custom_fields']??[]));return$id;}
    ...
    private function uniqueSlug(string$slug,int$ignore=0):string{global $wpdb;$members=$wpdb->prefix.'stageart_plugin_members';$history=$wpdb->prefix.'stageart_plugin_member_slug_history';$base=$slug?:'member';$c=$base;$i=2;while((int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$members} WHERE slug=%s AND id<>%d",$c,$ignore))||(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$history} WHERE slug=%s",$c)))$c=$base.'-'.($i++);return$c;}
